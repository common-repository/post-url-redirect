 ...
 * @author Haziq Khatri
 */
final class Pur_Redirector_Admin {
	...
	public function __construct()
	{
		...
		add_action( 'admin_init', array( $this, 'Pur_admin_reidrector_init' ) );
		add_action( 'admin_print_scripts-post.php', array( $this, 'Pur_css_js_enqueue_script' ) );
		add_action( 'admin_print_scripts-post-new.php', array( $this, 'Pur_css_js_enqueue_script' ) );
		add_action( 'admin_print_styles-post.php', array( $this, 'Pur_enqueue_style' ) );
		add_action( 'admin_print_styles-post-new.php', array( $this, 'Pur_enqueue_style' ) );
		add_action( 'plugins_loaded', array( $this, 'Pur_load_plugin_text_domain' ) );
		add_action( 'plugins_loaded', array( $this, 'Pur_maybe_update_plugin' ) );
		add_action( 'save_post', array( $this, 'Pur_save_post' ) );
		add_action( 'wp_ajax_redirector-search-posts', array( $this, 'Pur_search_posts' ) );
	 ...
	 * @author Haziq Khtri
	 **/
	public function Pur_admin_reidrector_init()
	{
		...
		foreach ( $post_types as $post_type ) :
			...
			add_meta_box( 'redirect', __( 'Redirect', 'redirector' ), array( $this, 'Pur_metabox' ), $post_type, 'side' );
	 ...
	 * @author Haziq Khatri
	 **/
	public function Pur_css_js_enqueue_script()
	{
	 ...
	 * @author Haziq Khatri
	 **/
	public function Pur_enqueue_style()
	{
	 ...
	 * @author Haziq Khatri
	 **/
	public function get_first_child_url_title( $post_id )
	{
	 ...
	 * @since v1.0.0
 	 * @author Haziq Khatri
	 **/
	protected function PUR_list_posts( $posts )
	{
		...
		<table class="widefat">
				...
				foreach ( $posts as $post ) :
					...
					<tr class="<?php echo esc_attr( $class) ?>">
						<th class="item-title">
							...
							<a href="<?php echo get_permalink( $post->ID ) ?>" target="_blank"><?php echo esc_url ( $title); ?></a>
							<?php if ( 'post' == $post->post_type ) :
								echo '<br><i>' . date_i18n( get_option( 'date_format' ), strtotime( $post->post_date ) ) . '</i>';
							endif;
							?>
						</th>
						<td class="item-info"><?php echo esc_attr( $post_types[$post->post_type]); ?></td>
						<td><a class="button select-redirector-post-id" href="#" data-id="<?php echo esc_attr($post->ID); ?>"><?php _e( 'Select', 'redirector' ); ?></a></td>
					</tr>
			...
			<tfoot>
				<tr>
					<th><?php _e( 'Title', 'redirector' ) ?></th>
					<th><?php _e( 'Type', 'redirector' ) ?></th>
					<th>&nbsp;</th>
				</tr>
			</tfoot>
		</table>
	 ...
	 * @author Haziq Khatri
	 **/
	public function Pur_load_plugin_text_domain()
	{
	 ...
	 * @author Haziq Khatri
	 **/
	public function Pur_maybe_update_plugin()
	{
	 ...
	 * @author Ralf Hortt
	 **/
	public function Pur_metabox( $post )
	{
		...
		<div class="redirector-redirect-type">
			...
			<div <?php if ( isset( $redirect['type'] ) && 'post' == $redirect['type'] ) echo esc_html('style="display:block;"'); ?>>

				<input type="hidden" id="redirector-post-id" name="redirector-post-id" value="<?php echo esc_url( $redirect_id ); ?>">
		...
		<div class="redirector-redirect-type">
			...
			<?php $url = ( isset( $redirect['url'] ) ) ?  apply_filters( 'redirector_url', $redirect['url'] ) : ''; ?>

			<label><input type="radio" name="redirect-type" value="url" <?php checked( $type, 'url' ) ?>> <?php _e( 'Website', 'redirector' ); ?></label>

			<div <?php if ( 'url' == $type )echo  esc_html( 'style="display:block;"'); ?>>

				<label>
					<?php _e( 'URL:', 'redirector' ); ?><br>
					<input name="redirector-url" value="<?php echo esc_url( $url);?>" type="url" />
				</label><br>

				<a id="redirector-url-preview" href="<?php echo esc_url( $url); ?>" target="_blank"><?php echo esc_url( $url); ?></a>
		...
		<div class="redirector-redirect-type">
			...
			<div <?php if ( 'first-child' == $type )  'style="display:block;"' ?>>

				<?php
				if ( FALSE !== ( $child_title = $this->get_first_child_url_title( $post->ID ) ) )
				echo esc_url( $child_title);
				else
		...
		<div class="redirector-redirect-type">
			...
			<div <?php if ( 'https' == $type ) echo esc_html('style="display:block;"'); ?>>

				<a href="<?php echo esc_url( str_replace( 'http:', 'https:', get_permalink( $post->ID ) ) ) ?>" target="_blank"><?php echo esc_attr($post->post_title); ?></a>
	...
	protected function modal_search( $redirect, $redirect_id )
	{
		...
		<div id="redirector-post-search">
			...
			<p>
				<input type="search" value="" id="redirector-search" placeholder="<?php _e( 'Search' ); ?>"> <a href="#" class="button" id="redirector-search-post"><?php _e( 'Search', 'redirector' ); ?></a>
			</p>
	 ...
	 * @author Haziq Khtri
	 **/
	public function Pur_save_post( $post_id )
	{

		if ( defined( 'PUR_DOING_AUTOSAVE' ) && PUR_DOING_AUTOSAVE )
			return;
			...
			switch ( $_POST['redirect-type'] ) :
				...
				case 'post' :
					...
					if ( '' != $_POST['redirector-post-id'] )
						$options['post_id'] = intval( sanitize_key($_POST['redirector-post-id']) );
					else
						unset( $options['type'] );
				...
				case 'first-child' :
					...
					if ( '' == $this->get_first_child_url_title( $post_id ) )
						unset( $options['type'] );
	 ...
	 * @author Haziq Khatri
	 **/
	public function Pur_search_posts()
	{
		...
		$this->PUR_list_posts( $query->posts );
...
} // end Pur_Redirector_Admin

new Pur_Redirector_Admin();
